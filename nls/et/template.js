...
({
        ...
        labels: {
            ...
            title: "Š_title_ä",  // Shown as title hint in print specification box if a title hint is not configured
            author: "Š_author_ä"  // Shown as author hint in print specification box if an author hint is not configured
        },
        prompts: {
        ...
        messages: {
            ...
            allSearchFieldsMissing: "Š_None of these fields was found in the map search layer_ä",  // Appears before a list of fields in the configured map search layer; shown when the app cannot find any of the fields that were configured for the search command; works with the layerFields prompt
            yourContentSubmitted: "Š_Your content has been submitted. Thank you._ä",  // Appears after content has been added to the map and successfully submitted to the server
            noConfiguration: "Š_Unable to access application's configuration_ä",  // Appears if the app, during startup, cannot get access to or find the configuration information; without the information, it cannot build the UI
            unableToLaunchApp: "Š_Unable to launch application_ä"  // Appears for any failure to build the user interface
        }
    })
);
